...
%% plot translation errors over distance
f = figure; hold on;
hb = bar(datasetsToRun,percentErrorOverTrajectory(:,conditionsToPlot),1);
set(hb(1),'facecolor','g'); 
set(hb(2),'facecolor','r'); 
set(hb(3),'facecolor','b'); 
set(hb(4),'facecolor','m');
...
xl = xlabel('datasets', 'FontSize', dim);
set(xl,'Interpreter','Latex');
ylabel('perc. translation error over traj. length', 'FontSize', dim);
set(gca, 'XTickLabel', datasetNames);
set(gca,'XTickLabelRotation',45);
%ylim([-0.01 0.15]);
if saveEps
    filename = horzcat(mainFolder,'/avePercTranErrOverTrajLen_vio_bars');
    saveas(f,filename,'epsc');
end
