...
# * Example:
# 	python config2sparkvio.py -config_option 'stereo-radtan' -input_cam /home/sb/ETH/example/camchain.yaml -input_imu /home/sb/ETH/example/imu.yaml -output /home/sb/ETH/example -responsible 'Sandro Berchier' -date '05.06.2019' -stereo 'MyntEye S' -IMU 'MyntEye S'
# * --------------------------------------------------------------------------
######################################### DEPENDENCIES ###########################################
...
if __name__ == "__main__":
	...
	parser.add_argument('-config_option','--config_option', help='VIO config option.', default='stereo-radtan')
	...
	parser.add_argument('-camera','--camera_hardware', help='Visual hardware of VI sensor.', default='unknown')
	...
	parser.add_argument('-baseline','--baseline', help='Baseline in m.',default=0.05)
	...
	if (args.config_option=='stereo-radtan'):
		...
		# Create config class
    		sparkvio_config = config_library.kalibr_configure_pinhole_radtan(args)

	elif (args.config_option=='RGBD-radtan'):

		# Create config class
    		sparkvio_config = config_library.kalibr_configure_RGBD_radtan(args)

	elif (args.config_option=='stereo-equi'):

		# Create config class
    		sparkvio_config = config_library.kalibr_configure_pinhole_equi(args)
	
	elif (args.config_option=='intel-radtan'):

		# Create config class
    		sparkvio_config = config_library.intel_configure_pinhole_radtan(args)
