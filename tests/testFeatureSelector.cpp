 ...
 * @brief  test FeatureSelector. FeatureSelector describes a greedy feature selection strategy, based on the paper:
 *
...
static const Pose3 body_P_leftCam  = Pose3( Rot3::Ypr(-M_PI / 2, 0., -M_PI / 2), Point3(0, 0, 1) );
static const Pose3 body_P_rightCam = body_P_leftCam.compose(Pose3(Rot3(),Point3(10,0,0))); // some baseline
...
TEST(FeatureSelector, cameras) {
  ...
  poses.push_back(StampedPose(Pose3(Rot3::Ypr(-M_PI, 0.1, -M_PI), Point3(1, 1, 1)),1));
...
TEST(FeatureSelector, createPrior1) {
  ...
  // and that the block structure of the covariance is what we ask for [pose, vel, biases]
  // we test two main things:
  // 1) the covariances of two poses connected by a (almost noiseless) between factor
  // are the same after rotating them to the global frame.
  // NOTE: one thing we do not capture is that the position uncertainty depends on the
  // rotation uncertainty, therefore this test is only working when the rotation uncertainty is relatively small
  // 2) check that WhitenInPlace works as we expect
  // 3) createPrior returns covariances in global frame

  // create toy example
  Pose3 pose0 = Pose3(Rot3::Ypr(-M_PI / 2, 0., -M_PI / 2), Point3(0, 0, 1) ); // some pose
  Pose3 pose1 = Pose3( Rot3::Ypr(M_PI , 0.1, 0.3), Point3(1, 10, 1) ); // some other pose
  Vector3 vel0 = Vector3(1,1,0);
  Vector3 vel1 = Vector3(5,3,1);
  imuBias::ConstantBias bias0 = imuBias::ConstantBias(Vector3(0.1,0.3,0.1), Vector3(0.2,0.8,0.1));
  imuBias::ConstantBias bias1 = imuBias::ConstantBias(Vector3(1.1,2.3,3.1), Vector3(1.2,1.8,1.1));
  ...
  SharedNoiseModel zeroVelocityPriorNoise = noiseModel::Diagonal::Sigmas(
      (Vector(3) << 7, 8, 9).finished());
  ...
  graph.push_back(boost::make_shared<BetweenFactor<Pose3> >(
      ...
      pose0.between(pose1), noiseModel::Isotropic::Sigma(6,smallSigma))); // very small noise

  // add between factors on vel
  graph.push_back(boost::make_shared<BetweenFactor<Vector3> >(
      Symbol('v', 0),Symbol('v', 1),
      vel1 - vel0, noiseModel::Isotropic::Sigma(3,smallSigma))); // very small noise

  // add between factors on biases
  graph.push_back(boost::make_shared<BetweenFactor<imuBias::ConstantBias> >(
      Symbol('b', 0),Symbol('b', 1),
      bias0.between(bias1), noiseModel::Isotropic::Sigma(6,smallSigma))); // very small noise
  ...
  // we expect this to be the same as the one on pose0 (since the between is almost noiseless)
  // but rotated in the frame of pose1:
  ...
  Matrix cov0 = UtilsOpenCV::Covariance_bvx2xvb(marginals.jointMarginalCovariance(keys0).fullMatrix());
  ...
  Matrix cov1 = UtilsOpenCV::Covariance_bvx2xvb(marginals.jointMarginalCovariance(keys1).fullMatrix());
  ...
  Matrix3 rot1 = pose1.rotation().matrix(); // (pose0.between(pose1)).rotation().matrix();
  ...
  noiseModel::Gaussian::shared_ptr noisePrior = noiseModel::Gaussian::Covariance(cov1_global);
  Matrix A = Matrix::Identity(15,15);
  noisePrior->WhitenInPlace(A); // This should make A = R = sqrt info matrix
  Matrix actualHessianMatrix = A.transpose() * A; // information = inverse-covariance
...
TEST(FeatureSelector, createMatricesLinearImuFactor) {
  ...
  StampedPose spose0 = StampedPose(Pose3(Rot3::Ypr(-M_PI / 2, 0., -M_PI / 2), Point3(0, 0, 1)), 0 );
  StampedPose spose1 = StampedPose(Pose3( Rot3::Ypr(M_PI , 0.1, 0.3), Point3(1, 10, 1)), 0.5);

  VioFrontEndParams trackerParams2 = VioFrontEndParams();
  trackerParams2.featureSelectionImuRate_ = 0.1; // fake imu rate to make problem simpler

  VioBackEndParams vioParams2 = VioBackEndParams();
  vioParams2.smartNoiseSigma_ = 1000;
  vioParams2.accNoiseDensity_ = 0.1; // fake noise to avoid that covariance it too small
  ...
  for(size_t k=0; k<5; k++) { // in 1 second we get 5 measurements at rate 0.1

    // Note: we integrate forward, hence at time j, we only have measurement of time j-1
    Matrix A_k_kp1 = Matrix::Zero(9,9); // system matrix for single time step
    A_k_kp1.block<3,9>(0,0) << I_3x3, delta * I_3x3,   - 0.5 * (delta*delta) * Rk;
  ...
  Ai_expected = -Ai_expected; // because we have to move it to the left-hand side

  // check that rotations are subdivided correctly: after integrating 10 times, we get Rj
  ...
  // check that building covariance incrementally gives the same result as batch creation
  // NOTE: we do not capture correlation with bias evolution hence we need to zero out those blocks
  // Same as difference between imu factor and combined Imu factor
...
TEST_UNSAFE(FeatureSelector, createOmegaBarImu) {
  ...
  StampedPose spose0 = StampedPose(Pose3(Rot3::Ypr(0.2,0.4,0.5), Point3(0, 0, 1)), 0 );
  StampedPose spose1 = StampedPose(Pose3( Rot3::Ypr(0.5,0.5,0.3), Point3(1, 10, 1)), 0.5);
  StampedPose spose2 = StampedPose(Pose3( Rot3::Ypr(0.1,0.2,0.3), Point3(0.1,0.1,0.1)),1);
...
TEST(FeatureSelector, GetVersorIfInFOV) {
  ...
  for(size_t i = 0; i < 4 ; i++){
    switch(i){
    case 0 : p = cam.backproject(Point2(1,1), depth); break;
    case 1 : p = cam.backproject(Point2(639,480), depth); break;
    case 2 : p = cam.backproject(Point2(320,200), depth); break;
    case 3 : p = cam.backproject(Point2(640,0), depth); break;
    }
  ...
  for(size_t i = 0; i < 4 ; i++){
    switch(i){
    case 0 : p = cam.backproject(Point2(-1,-1), depth); break;
    case 1 : p = cam.backproject(Point2(641,480), depth); break;
    case 2 : p = cam.backproject(Point2(-320,200), depth); break;
    case 3 : p = cam.backproject(Point2(641,0), depth); break;
    }
    // check that points in FOV are calibrated correctly
...
TEST(FeatureSelector, createLinearVisionFactor_no_parallax) {
  ...
  // create a set of Jacobian factors, then eliminate points and get the same matrix
  Camera cam(spose0.pose,K);
  Point3 pworld_l = cam.backproject(Point2(320,200), 2.0); // backprojected 2 meters away
  ...
  HessianFactor::shared_ptr hFactor = f.createLinearVisionFactor(pworld_l,left_cameras,right_cameras,time1, time2, time3);
...
TEST(FeatureSelector, createLinearVisionFactor_And_SchurComplement) {
  ...
  Pose3 pose0 = Pose3(Rot3::Ypr(0.2,0.4,0.5), Point3(0, 0, 1));
  StampedPose spose0 = StampedPose(pose0 , 0 );
  StampedPose spose1 = StampedPose(pose0.compose(Pose3(Rot3(),Point3(0.2,0,0))) , 0.5);
  StampedPose spose2 = StampedPose(pose0.compose(Pose3(Rot3(),Point3(0.5,0,0))) ,1);
  ...
  double time1 = 0, time2 = 0, time3 = 0;
  HessianFactor::shared_ptr hFactor = f.createLinearVisionFactor(pworld_l,left_cameras,right_cameras,time1, time2, time3);
  Matrix actualHessian = hFactor->information(); // NOTE: he we assume that the keys are in the right order

  // create graph Jacobian factors corresponding to projection measurements:
  GaussianFactorGraph gfg;
  Key l0 = 10; // a key
  for(size_t i = 0; i < 3; i++){ // for each camera we create 2 jacobian factors (left, right)
  ...
  Matrix expectedHessian =
      F.transpose() * F - F.transpose() * E * (E.transpose() * E).inverse() * E.transpose() * F;
...
/* ************************************************************************* */
// helper function
Matrix schurComplementTest(Point3 pworld_l, vector<StampedPose> posesAtFutureKeyframes,
    Pose3 b_P_LCam, Pose3 b_P_RCam) {
  // create graph Jacobian factors corresponding to projection measurements:
  GaussianFactorGraph gfg;
  Key l0 = 10; // a key
  for(size_t i = 0; i < posesAtFutureKeyframes.size(); i++){ // for each camera we create 2 jacobian factors (left, right)
    for(size_t j=0; j<2; j++){ // for left and right
...
TEST(FeatureSelector, createLinearVisionFactor_body_P_cam) {
  ...
  const Pose3 b_P_LCam  = Pose3( Rot3::Ypr(-M_PI / 2, 0., -M_PI / 2), Point3(0, 0, 1) );
  const Pose3 b_P_RCam = b_P_LCam.compose(Pose3(Rot3(),Point3(0.1,0,0))); // some baseline
  ...
  // check that points in FOV are calibrated correctly
  EXPECT(FeatureSelector::GetVersorIfInFOV(Camera(spose1.pose.compose(b_P_LCam),K),pworld_l));
  EXPECT(FeatureSelector::GetVersorIfInFOV(Camera(spose2.pose.compose(b_P_LCam),K),pworld_l));
  EXPECT(FeatureSelector::GetVersorIfInFOV(Camera(spose1.pose.compose(b_P_RCam),K),pworld_l));
  EXPECT(FeatureSelector::GetVersorIfInFOV(Camera(spose2.pose.compose(b_P_RCam),K),pworld_l));

  double time1 = 0, time2 = 0, time3 = 0;
  HessianFactor::shared_ptr hFactor = f.createLinearVisionFactor(pworld_l,left_cameras,right_cameras,time1, time2, time3);
  Matrix actualHessian = hFactor->information(); // NOTE: he we assume that the keys are in the right order

  Matrix expectedHessian = schurComplementTest(pworld_l,featureSelectionData.posesAtFutureKeyframes,
      b_P_LCam,b_P_RCam);
...
GaussianFactorGraph::shared_ptr createOmegaBarTest(){
  ...
  Pose3 pose0 = Pose3(Rot3::Ypr(0.2,0.4,0.5), Point3(0, 0, 1));
  StampedPose spose0 = StampedPose(pose0 , 0 );
  StampedPose spose1 = StampedPose(pose0.compose(Pose3(Rot3::Ypr(0.02,0.04,0.05),Point3(0.2,0,0))) , 0.5);

  // create featureSelectionData
  FeatureSelectorData featureSelectionData;
  featureSelectionData.posesAtFutureKeyframes.push_back(spose0);
  featureSelectionData.posesAtFutureKeyframes.push_back(spose1);
  featureSelectionData.currentNavStateCovariance = 0.0001 * Matrix::Identity(15,15);
  featureSelectionData.left_undistRectCameraMatrix = K;
  featureSelectionData.right_undistRectCameraMatrix = K;
  Camera cam(spose0.pose,K);
  Point3 pworld_l = cam.backproject(Point2(320,200), 2.0); // backprojected 2 meters away
  featureSelectionData.keypoints_3d.push_back(pose0.transform_to(pworld_l)); // convert to local frame
...
TEST(FeatureSelector, createOmegaBar) {
  ...
  // get gaussian factor graph by calling createOmegaBar (done inside function to avoid more copy-paste)
  GaussianFactorGraph::shared_ptr gfg = createOmegaBarTest();

  // check keys:
  FastVector<Key> keys0 = gfg->at(0)->keys();
  EXPECT(keys0.size() == 2);
  EXPECT(keys0[0] == 0 && keys0[1] == 1); // single factor, including imu, prior, and vision
}
...
TEST(FeatureSelector, evaluateGain_det ) {
  ...
  JacobianFactor J = JacobianFactor(
      0, 1 * Matrix::Identity(3,9),
      1, 1 * Matrix::Identity(3,9), Vector3::Zero());
  ...
  // test with actual hessian factor
  {
    ...
    for(size_t denseOrNot = 0; denseOrNot < 2; denseOrNot++){
      if(denseOrNot == 1)
        useDenseMatrices = false;

      // actual
      double actualDet = FeatureSelector::EvaluateGain(gfg,H,
          VioFrontEndParams::FeatureSelectionCriterion::LOGDET,useDenseMatrices);
      EXPECT_DOUBLES_EQUAL(expectedLogDet,actualDet,expectedLogDet*1e-3); // relative tolerance
      // compare against matlab determinant:
      EXPECT_DOUBLES_EQUAL(log(5.591685310658876e+79),actualDet,expectedLogDet*1e-3); // determinant should be large

      // actual2: call it again and make sure we did not mess up gfg inside the function
      actualDet = FeatureSelector::EvaluateGain(gfg,H,
          VioFrontEndParams::FeatureSelectionCriterion::LOGDET,useDenseMatrices);
      EXPECT_DOUBLES_EQUAL(expectedLogDet,actualDet,expectedLogDet*1e-3); // relative tolerance
      EXPECT_DOUBLES_EQUAL(log(5.591685310658876e+79),actualDet,expectedLogDet*1e-3); // determinant should be large
    }
  ...
  // test with empty hessian factor
  {
    ...
    for(size_t denseOrNot = 0; denseOrNot < 2; denseOrNot++){
      ...
      // actual
      double actualDet = FeatureSelector::EvaluateGain(gfg,
...
TEST(FeatureSelector, evaluateGain_minEig ) {
  ...
  // test with actual hessian factor
  {
    ...
    int rank; double expectedMinEig; Vector eigVector;
    boost::tie(rank,expectedMinEig,eigVector) = DLT(expectedHessian);
    EXPECT_DOUBLES_EQUAL(2.990918403930777e+03,expectedMinEig,expectedMinEig*1e-4);// relative tolerance
    ...
    for(size_t denseOrNot = 0; denseOrNot < 2; denseOrNot++){
      ...
      // actual
      double actualMinEig = FeatureSelector::EvaluateGain(gfg,H,
          VioFrontEndParams::FeatureSelectionCriterion::MIN_EIG,useDenseMatrices);
      EXPECT_DOUBLES_EQUAL(expectedMinEig,actualMinEig,expectedMinEig*1e-4); // relative tolerance

      // actual2: call it again and make sure we did not mess up gfg inside the function
      actualMinEig = FeatureSelector::EvaluateGain(gfg,H,
          VioFrontEndParams::FeatureSelectionCriterion::MIN_EIG,useDenseMatrices);
      EXPECT_DOUBLES_EQUAL(expectedMinEig,actualMinEig,expectedMinEig*1e-4); // relative tolerance
    }
  }
  // test with empty hessian factor
  {
    ...
    for(size_t denseOrNot = 0; denseOrNot < 2; denseOrNot++){
      ...
      // actual
      double actualMinEig = FeatureSelector::EvaluateGain(gfg,
          boost::make_shared<HessianFactor>(),
          VioFrontEndParams::FeatureSelectionCriterion::MIN_EIG,useDenseMatrices);
      EXPECT_DOUBLES_EQUAL(expectedMinEig,actualMinEig,expectedMinEig*1e-4); // relative tolerance
    }
...
TEST(FeatureSelector, greedyAlgorithm ) {
  ...
  for(size_t i=0; i<10; i++){
    ...
    Deltas.push_back(
        boost::make_shared<HessianFactor>(
            HessianFactor(0, 1,
                c * Matrix::Identity(9,9), Matrix::Zero(9,9), Vector::Zero(9),
  ...
  vector<size_t> actualEig; vector<double> actualGainEig;
  tie(actualEig,actualGainEig) =
      FeatureSelector::GreedyAlgorithm(OmegaBar,Deltas,need_n_corners,
  ...
  vector<size_t> actualDet; vector<double> actualGainDet;
  tie(actualDet,actualGainDet) =
      FeatureSelector::GreedyAlgorithm(OmegaBar,Deltas,need_n_corners,
...
TEST(FeatureSelector, createDeltas) {
  ...
  Pose3 pose0 = Pose3(Rot3::Ypr(0.2,0.4,0.5), Point3(0, 0, 1));
  StampedPose spose0 = StampedPose(pose0 , 0 );
  StampedPose spose1 = StampedPose(pose0.compose(Pose3(Rot3(),Point3(0.3,0,0))) , 0.5);
  StampedPose spose2 = StampedPose(pose0.compose(Pose3(Rot3(),Point3(0.4,0,0))) , 1);
  ...
  Camera cam(spose0.pose,K);
  Point3 pworld_l = cam.backproject(Point2(320,200), 2.0); // backprojected 2 meters away
  featureSelectionData.keypoints_3d.push_back(spose0.pose.transform_to(pworld_l)); // convert to local frame
  featureSelectionData.keypointLife.push_back(3);
  ...
  Vector3 v1 = K.calibrate(Vector3(322,201,1)); v1=v1/v1.norm();
  availableVersors.push_back(v1); // close to existing keypoint
  Vector3 v2 = K.calibrate(Vector3(400,400,1)); v2=v2/v2.norm();
  ...
  vector<HessianFactor::shared_ptr> Deltas =
      f.createDeltas(availableVersors,cornerDistances,featureSelectionData,left_cameras, right_cameras);

  EXPECT(Deltas.size()==2); // 2 factors, 1 for each point

  // NOTE: WE ARE ASSUMING DISTANCE, NOT DEPTH: see the following to understand difference
  // first point is close to existing keypoint, hence the depth should be the same as that point
  Point3 p1depth = cam.backproject(Point2(322,202), 2.0);
  // check 2 types of reprojection
  Point3 p1depth_check = left_cameras.at(0).pose() * Point3(availableVersors[0] * 2.0 / availableVersors[0](2));
  EXPECT(assert_equal(p1depth,p1depth_check,1e-2)); // strangely we need some tolerance here

  // however we assume norm, rather than depth:
  Point3 p1 = left_cameras.at(0).pose() * Point3(availableVersors[0] * 2.0);
  // check hessian
  Matrix expectedHessian1 = schurComplementTest(p1,featureSelectionData.posesAtFutureKeyframes,Pose3(),Pose3());
  ...
  // second point is far from existing keypoint, hence the distance should be the default = 5
  Point3 p2 = left_cameras.at(0).pose() * Point3(availableVersors[1] * 5.0);
  Matrix expectedHessian2 = schurComplementTest(p2,featureSelectionData.posesAtFutureKeyframes,Pose3(),Pose3());
...
TEST(FeatureSelector, featureSelection ) {
  ...
  Pose3 pose0 = Pose3();
  StampedPose spose0 = StampedPose(pose0, 0 );
  StampedPose spose1 = StampedPose(pose0.compose(Pose3( Rot3(),Point3(0, 1, 0))), 0.5);
  StampedPose spose2 = StampedPose(pose0.compose(Pose3( Rot3(),Point3(0, 2, 0))),  1);

  // camera estrinsics: for left camera to have z forward, we need a -90deg rotation along x
  ...
  EXPECT(assert_equal(Point2(Kreal.px(),Kreal.py()), cam0.project(Point3(0,1,0)) ));
  ...
  // 2) since the initial pose is the identity, no translation occurs and uncertainty remains along x

  // create camera params
  CameraParams cam_param;
  cam_param.calibration_ = Cal3DS2(Kreal.fx(), Kreal.fy(), 0.0, Kreal.px(), Kreal.py(), 0.0, 0.0); // 0 skew and no distortion
  ...
  VioBackEndParams vp = VioBackEndParams();
  vp.smartNoiseSigma_ = 1000;
  vp.accNoiseDensity_ = 0.1; // fake noise to avoid that covariance it too small
  ...
  // TEST 1
  {
    ...
    KeypointsCV selected; vector<size_t> selectedIndices;
    ...
    tie(selected,selectedIndices,selectedGains) = f.featureSelectionLinearModel(availableCorners,
        successProbability, cornerDistances, cam_param, need_n_corners,
        featureSelectionData,VioFrontEndParams::FeatureSelectionCriterion::MIN_EIG);
    ...
    tie(selected,selectedIndices,selectedGains) = f.featureSelectionLinearModel(availableCorners,
        successProbability, cornerDistances, cam_param, need_n_corners,
        featureSelectionData,VioFrontEndParams::FeatureSelectionCriterion::LOGDET);
    EXPECT_DOUBLES_EQUAL(1,selectedIndices[0],1e-3);
    EXPECT(selectedIndices.size()==1);
    EXPECT(selected.size()==1);
  }
...
TEST(FeatureSelector, sorting ) {
  ...
  tie(ordering,sortedUpperBounds_actual) = FeatureSelector::SortDescending(upperBounds);
...
TEST(FeatureSelector, MultiplyHessianInPlace ) {
  ...
  dims.push_back(2); dims.push_back(4); dims.push_back(1);
  ...
  // multiply by 2:
  {
    ...
    HessianFactor::shared_ptr expected =
        boost::make_shared<HessianFactor>(keys, SymmetricBlockMatrix(dims, 2*M));
  ...
  // multiply by 0:
  {
    ...
    HessianFactor::shared_ptr expected =
        boost::make_shared<HessianFactor>(keys, SymmetricBlockMatrix(dims, Matrix::Zero(7,7)));
    // the following fails since we are not multiplying the vector
    EXPECT(assert_equal(*expected.get(),*h.get(),1e-4));
  }
  // multiply by 100.01 :-)
  {
    ...
    HessianFactor::shared_ptr expected =
        boost::make_shared<HessianFactor>(keys, SymmetricBlockMatrix(dims, 100.01 * M));
    // the following fails since we are not multiplying the vector
    EXPECT(assert_equal(*expected.get(),*h.get(),1e-4));
  }
}
...
TEST(FeatureSelector, smallestEigSpectra ) {
  ...
  double expectedEig; Vector expectedVect; int expectedRank;
  boost::tie(expectedRank,expectedEig,expectedVect) = FeatureSelector::SmallestEigs(M);

  double actualEig; Vector actualVect; int actualRank;
  boost::tie(actualRank,actualEig,actualVect) = FeatureSelector::SmallestEigsSpectra(M);

  EXPECT_DOUBLES_EQUAL(expectedEig,actualEig,fabs(expectedEig)*1e-3); // relative tolerance
  EXPECT(assert_equal(expectedVect,actualVect,1e-2));
}
...
TEST(FeatureSelector, smallestEigSpectraShift ) {
  ...
  double actualEig; Vector actualVect; int actualRank;
  boost::tie(actualRank,actualEig,actualVect) = FeatureSelector::SmallestEigsSpectraShift(M);
