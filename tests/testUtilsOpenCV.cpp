...
static const double poseArray[] = {0.0148655429818, -0.999880929698, 0.00414029679422, -0.0216401454975,
...
static const Mat R_cvmat = (cv::Mat_<double>(3, 3) << poseArray[0], poseArray[1], poseArray[2],
    poseArray[4], poseArray[5], poseArray[6], poseArray[8], poseArray[9], poseArray[10]);
static const Mat T_cvmat = (cv::Mat_<double>(3, 1) << poseArray[3], poseArray[7], poseArray[11]);

/* ------------------------------------------------------------------------ */
// helper function
static pair<cv::Mat, vector<cv::Point2f>> cvCreateChessboard(int squareWidth, int numPatternRow, int numPatternCol) {
  ...
  for (int pr = 1; pr < numPatternRow; pr++) {
    for (int pc = 1; pc < numPatternCol; pc++) {
      corners.push_back(cv::Point2f(pc * squareWidth - 0.5, pr * squareWidth - 0.5));
    }
  }
...
TEST(testUtils, CvMatCmp){
  ...
  EXPECT(UtilsOpenCV::CvMatCmp(chessboardImg,chessboardImg));
  // but becomes different if we perturb an entry
  Mat chessboardImg2;
  chessboardImg.copyTo(chessboardImg2);
  // perturb some entry:
  chessboardImg2.at<unsigned char>(0, 1) = chessboardImg2.at<unsigned char>(0, 1) + 1;
  EXPECT(!UtilsOpenCV::CvMatCmp(chessboardImg,chessboardImg2));
}
...
TEST(testUtils, CvPointCmp){
  ...
  EXPECT(UtilsOpenCV::CvPointCmp(p1,p1));
  // but becomes different if we perturb an entry
  Point2f p2(2.0 + 1e-5,1.5);
  EXPECT(!UtilsOpenCV::CvPointCmp(p1,p2,1e-7));
  // however we cannot detect differences smaller than our tolerance:
  Point2f p3(2.0 + 1e-8,1.5);
  EXPECT(UtilsOpenCV::CvPointCmp(p1,p3,1e-7));
}
...
TEST(testUtils, Vec2pose) {
  ...
  // Comparison!
  EXPECT(assert_equal(pose_expected,pose_actual));
}
...
TEST(testUtils, Pose2cvmats) {
  ...
  // Comparison
  EXPECT(UtilsOpenCV::CvMatCmp(R_expected, R_actual));
  EXPECT(UtilsOpenCV::CvMatCmp(T_expected, T_actual));
}
...
TEST(testUtils, Pose2Affine3d) {
  ...
  // Comparison
  for (int r = 0; r < 3; r++) {
    EXPECT_DOUBLES_EQUAL(T_expected.at<double>(r, 0),
        double (affineActual.translation()(r)), tol);
    for (int c = 0; c < 3; c++) {
      EXPECT_DOUBLES_EQUAL(R_expected.at<double>(r, c),
...
TEST(testUtils, Cvmat2rot) {
  ...
  EXPECT(assert_equal(Rot_expected,Rot_actual));
}

/* ************************************************************************* */
TEST(testUtils, Cvmat2Cal3_S2) {
  // input CV Mat from MH_easy_01/cam0 (modified)
  Mat M = (cv::Mat_<double>(3, 3) << 458.654, 0.01, 367.215,
      0, 457.296, 248.375,
      0, 0, 1);
  // Expected output
  ...
  EXPECT(assert_equal(cal_expected,cal_actual));
}
...
TEST(testUtils, Cal3_S2ToCvmat) {
  ...
  // Actual output
  EXPECT(UtilsOpenCV::CvMatCmp(M,Mactual,1e-5));
}
...
TEST(testUtils, Gvtrans2pose) {
  ...
  RT << poseArray[0], poseArray[1], poseArray[2], poseArray[3],
...
TEST(testUtils, CropToSizeInside) {
  ...
  EXPECT(UtilsOpenCV::CvPointCmp(px_expected, px_actual));
}
...
TEST(testUtils, CropToSize) {
  ...
  { // check that point inside boundary remains the same
    ...
    // Crop (699.50001f, 300.499f) in the frame of 800w*600h. Should output(700, 300)
...
TEST(testUtils, RoundToDigit_2digits ) {
  ...
  EXPECT_DOUBLES_EQUAL(x_expected,x_actual,tol);
}
...
TEST(testUtils, RoundUnit3 ) {
  ...
  EXPECT(assert_equal(a_expected,a_actual,tol));
  }
  {
  Unit3 a(0,-1,0.9);
  Unit3 a_actual = UtilsOpenCV::RoundUnit3(a);
...
TEST(testUtils, To_string_with_precision_pos4digits) {
  ...
  EXPECT(str_expected.compare(str_actual) == 0);
}
...
TEST(testUtils, NsecToSec) {
  ...
  EXPECT_DOUBLES_EQUAL(sec_expected, sec_actual, 1e-10);
}
...
TEST(testUtils, computeRTErrors_identical) {
  ...
  tie(rot_error_actual, tran_error_actual) = UtilsOpenCV::ComputeRotationAndTranslationErrors(pose_gtsam, pose_gtsam);

  EXPECT_DOUBLES_EQUAL(rot_error_expected, rot_error_actual, tol);
  EXPECT_DOUBLES_EQUAL(tran_error_expected, tran_error_actual, tol);
}
...
TEST(testUtils, computeRTErrors) {
  ...
  tie(rot_error_actual, tran_error_actual) = UtilsOpenCV::ComputeRotationAndTranslationErrors(pose_gtsam, pose_new);
...
TEST(testUtils, computeRTErrors_upToScale) {
...

  {
    ...
    double rot_error_actual, tran_error_actual;
    Pose3 pose_new(R_new, T_new);
    tie(rot_error_actual, tran_error_actual) = UtilsOpenCV::ComputeRotationAndTranslationErrors(pose_gtsam, pose_new, upToScale);
  ...
  {
    // translation of new pose is the same of the expected, but with some other norm
    double normT = pose_gtsam.translation().vector().norm();
...
TEST(testUtils, ReadAndConvertToGrayScale) {
  // original image is already gray, hence it remains the same
  {
    ...
    vector<cv::Point2f> keypoints_expected;
    tie(chessboardImg, keypoints_expected) = cvCreateChessboard(30, 10, 8);
    Mat chessboardImgGray = UtilsOpenCV::ReadAndConvertToGrayScale("chessboard.png");
    EXPECT(chessboardImgGray.channels() == 1);
    EXPECT(UtilsOpenCV::CvMatCmp(chessboardImg, chessboardImgGray));
  }
  // original image is in color and it is converted to gray
  {
    Mat imageGray = UtilsOpenCV::ReadAndConvertToGrayScale(string(DATASET_PATH) + "lena.png");
    ...
    EXPECT(UtilsOpenCV::CvMatCmp(imageGray, imgGrayWritten));
  }
  // original image is already gray, hence it remains the same
  {
    Mat img = imread(string(DATASET_PATH) + "testImage.png", IMREAD_ANYCOLOR);
    Mat imageGray = UtilsOpenCV::ReadAndConvertToGrayScale(string(DATASET_PATH) + "testImage.png");
    EXPECT(imageGray.channels() == 1);
...
TEST(testUtils, covariance_bvx2xvb) {
  ...
  EXPECT(assert_equal(expected_cov_xvb, cov_actual_xvb));
}
...
TEST(UtilsOpenCV, ExtractCornersChessboard) {
  ...
  EXPECT_DOUBLES_EQUAL(numCorners_expected, actualCorners2.size(), 1e-3);
...
TEST(UtilsOpenCV, VectorUnique){
  ...
  EXPECT(vexpected.size() == vactual.size());
