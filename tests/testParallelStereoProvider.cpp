...
/**
 * @file   tstParallelStereoProvider.cpp
...
class TestParallelStereoProvider : public ::testing::Test {
 public:
  ...
  ~TestParallelStereoProvider() = default;
...
TEST_F(TestParallelStereoProvider, basicParallelCase) {
...
TEST_F(TestParallelStereoProvider, dropFramesOlderThanImu) {
  addImu(10);
  addFrame(11);
  EXPECT_TRUE(output_queue_.empty());
...
TEST_F(TestParallelStereoProvider, imageBeforeImuTest) {
...
TEST_F(TestParallelStereoProvider, testOutOfOrderImuAndImageSequence) {
  addImu(0);  // Get past the need for available IMU data
  addFrame(3);
  addImu(2);
  addImu(4);
