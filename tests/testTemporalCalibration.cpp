...
TestData makeTestData(size_t num_frames = 10,
  ...
  for (size_t i = 1; i <= num_frames; ++i) {
    ...
    for (size_t k = 0; k <= num_imu_per; ++k) {
      ...
      values(3, k) = ratio * value_diff + prev_angle;
      values(3, k) /= to_return.params.nominal_sampling_time_s_;
    }
    prev_angle = angle;

    // this is actually a different axis, but the transform doesn't matter
    gtsam::Pose3 pose(gtsam::Rot3::Rz(angle), Eigen::Vector3d::Zero());
    to_return.results.emplace_back(TrackingStatus::VALID, pose);
    to_return.outputs.emplace_back(makeOutput(num_imu_per * i));
  ...
  for (size_t i = 0; i <= results.size(); ++i) {
    ...
    if (i < results.size()) {
      ...
      EXPECT_LE(UtilsNumerical::NsecToSec(-results.size() + 1),
                result.imu_time_shift);
      // we have a stable max search, so no time-shift wins out
