...
double UtilsGeometry::getRatioBetweenTangentialAndRadialDisplacement(
  ...
  double max_t = 0.0;
  double tangential_elongation_1 = (points_rescaled.at(0) -
                                    points_rescaled.at(1)).norm();
  max_t = std::max(max_t, tangential_elongation_1);
  double tangential_elongation_2 = (points_rescaled.at(1) -
                                    points_rescaled.at(2)).norm();
  max_t = std::max(max_t, tangential_elongation_2);
  double tangential_elongation_3 = (points_rescaled.at(0) -
                                    points_rescaled.at(2)).norm();
