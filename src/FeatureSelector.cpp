...
using KeyframeToStampedPose = std::vector<StampedPose>; // map future keyframe id to StampedPose, with the assumption that
...
void FeatureSelectorData::print() const {
  ...
  for(size_t i=0; i < posesAtFutureKeyframes.size(); i++){
    std::cout << "\n timestampInSec: " << posesAtFutureKeyframes.at(i).timestampInSec << std::endl;
  ...
  for(size_t i=0; i<keypoints_3d.size(); i++ ){
    std::cout << "keypoints_3d: " << keypoints_3d.at(i).transpose() << " with life " << keypointLife.at(i) << std::endl;
  }
}
...
void FeatureSelector::MultiplyHessianInPlace(
  ...
  gtsam::Matrix B;
  for (size_t i = 0; i < keys.size()+1; i++) { // +1 since there is also a vector attached
    B = cH.block(i,i);
    cH.setDiagonalBlock(i, c * B);
    for (size_t j = i + 1; j < keys.size()+1; j++) { // +1 since there is also a vector attached
      B = cH.block(i,j);
...
FeatureSelector::featureSelectionLinearModel(const KeypointsCV& availableCorners,
  ...
  if(debug) {std::cout << "featureSelectionLinearModel: getCameras" << std::endl;}
  Cameras left_cameras, right_cameras;
  std::tie(left_cameras, right_cameras) = getCameras(featureSelectionData);

#ifdef FEATURE_SELECTOR_DEBUG_COUT
  std::cout << "getCameras time: " << UtilsOpenCV::GetTimeInSeconds() - startTime << std::endl;
  startTime = UtilsOpenCV::GetTimeInSeconds();
#endif

  // create OmegaBar: includes IMU and existing vision measurements
  if(debug) {std::cout << "featureSelectionLinearModel: createOmegaBar" << std::endl;}
  gtsam::GaussianFactorGraph::shared_ptr OmegaBar = createOmegaBar(featureSelectionData,left_cameras,right_cameras);

#ifdef FEATURE_SELECTOR_DEBUG_COUT
  std::cout << "OmegaBar time: " << UtilsOpenCV::GetTimeInSeconds() - startTime << std::endl;
  startTime = UtilsOpenCV::GetTimeInSeconds();
#endif

  // get directions from each (UNCALIBRATED) available corner in left camera at time 0 (cam_param includes also distortion)
  if(debug) {std::cout << "featureSelectionLinearModel: availableVersors" << std::endl;}
  std::vector<gtsam::Vector3> availableVersors;  availableVersors.reserve(availableCorners.size());
  for(size_t l=0; l < availableCorners.size(); l++)
    availableVersors.push_back(Frame::CalibratePixel(availableCorners.at(l),cam_param));

#ifdef FEATURE_SELECTOR_DEBUG_COUT
  std::cout << "known points time: " << UtilsOpenCV::GetTimeInSeconds() - startTime << std::endl;
  startTime = UtilsOpenCV::GetTimeInSeconds();
#endif

  // create Deltas for each direction
  if(debug) {std::cout << "featureSelectionLinearModel: createDeltas" << std::endl;}
  std::vector<gtsam::HessianFactor::shared_ptr> Deltas =
      createDeltas(availableVersors, availableCornersDistances, featureSelectionData, left_cameras, right_cameras);

#ifdef FEATURE_SELECTOR_DEBUG_COUT
  std::cout << "createDeltas time: " << UtilsOpenCV::GetTimeInSeconds() - startTime <<
               "(nr factors, maybe empty: " << Deltas.size() << ")"<< std::endl;
  startTime = UtilsOpenCV::GetTimeInSeconds();
#endif

  if(useSuccessProbabilities_){
    // UtilsOpenCV::PrintVector(successProbabilities,"successProbabilities in selector");
...
#ifdef FEATURE_SELECTOR_DEBUG_COUT
  std::cout << "success time: " << UtilsOpenCV::GetTimeInSeconds() - startTime << std::endl;
  startTime = UtilsOpenCV::GetTimeInSeconds();
#endif

  // apply greedy algorithm to select need_n_corners out of the available corners
  if(debug) {std::cout << "featureSelectionLinearModel: GreedyAlgorithm" << std::endl;}
  std::vector<size_t> selectedIndices; std::vector<double> selectedMarginalGains;
  std::tie(selectedIndices,selectedMarginalGains) = GreedyAlgorithm(OmegaBar,Deltas,need_n_corners,criterion, useLazyEvaluation_);

#ifdef FEATURE_SELECTOR_DEBUG_COUT
  std::cout << "Overall time greedy alg: " << UtilsOpenCV::GetTimeInSeconds() - startTime << std::endl;
#endif

  if(debug) {std::cout << "featureSelectionLinearModel: selectedCorners" << std::endl;}
  KeypointsCV selectedCorners; selectedCorners.reserve(selectedIndices.size());
...
FeatureSelector::SortDescending(const std::vector<double>& upperBounds) {
  ...
  std::vector<std::pair<size_t,double> > indValuePairs; indValuePairs.reserve(N);
  ...
  std::vector<size_t> ordering; ordering.reserve(N);
  std::vector<double> sortedUpperBound; sortedUpperBound.reserve(N);
  for(size_t j = 0; j<N; j++){
...
FeatureSelector::SmallestEigs(const gtsam::Matrix& A) {
...
FeatureSelector::OrderByUpperBound(const gtsam::GaussianFactorGraph::shared_ptr& bestOmegaBar,
  ...
  int rank; double eigValue; gtsam::Vector eigVector;
  double gainOmegaBar;
  ...
  switch(criterion)
  {
    case VioFrontEndParams::FeatureSelectionCriterion::MIN_EIG :
      ...
      boost::tie(rank,eigValue,eigVector) = SmallestEigs(bestOmegaBar->hessian().first);
      gainOmegaBar = eigValue;
      for(auto key : bestOmegaBar->keys()){
        xx.insert(key, eigVector.segment<9>(9*key));
      }
      // compute upper bounds lambdaMin(A + B) = lambdaMin(A) + norm(B * mu_min_A)
      for(size_t j=0; j<Deltas.size(); j++){
        yy.setZero();
      ...
      break;
    case VioFrontEndParams::FeatureSelectionCriterion::LOGDET :  // picks the best features that maximize the logdet of the covariance
      gainOmegaBar = EvaluateGain( bestOmegaBar, boost::make_shared<gtsam::HessianFactor>(), criterion);
      // compute upper bounds det(M) = prod (Mii)
      for(size_t j=0; j<Deltas.size(); j++){ // for each delta
        if(!Deltas.at(j)->empty())
          bestOmegaBar->push_back(Deltas.at(j));
        gtsam::Vector hessianDiagonal = bestOmegaBar->hessianDiagonal().vector();
        double sumLogDiag = 0; for(size_t k=0; k<hessianDiagonal.size(); k++){ sumLogDiag+=log(hessianDiagonal(k)); }
        upperBounds.push_back( sumLogDiag );
        bestOmegaBar->resize(sizeBestOmegaBar); // since bestOmegaBar is a shared pointer, it would carry Deltas.at(j)
      }
      break;
    default: throw std::runtime_error("OrderByUpperBound: wrong choice of criterion"); break;
  }
  // check that upper bounds are valid
  if(testing){
    for(size_t j=0; j<Deltas.size(); j++){
      std::cout.precision(20);
      std::cout << "upb: " << upperBounds.at(j) << " gain: " << EvaluateGain( bestOmegaBar, Deltas.at(j), criterion)
                << " criterion " << criterion << std::endl;
      // check with some relative (numerical) tolerance
      if(upperBounds.at(j) < (1-1e-7) * EvaluateGain( bestOmegaBar, Deltas.at(j), criterion)){
        std::cout << "WATCH OUT : upb: " << upperBounds.at(j) << " gain: " <<
                     EvaluateGain( bestOmegaBar, Deltas.at(j), criterion) << " criterion " << criterion << std::endl;
        throw std::runtime_error("OrderByUpperBound: invalid upper bound");
      }
    }
  }
  // sort in descending order by upper bound
  std::vector<size_t> ordering; std::vector<double> sortedUpperBounds;
...
/* ------------------------------------------------------------------------ */
std::pair< std::vector<size_t>,std::vector<double> >
FeatureSelector::GreedyAlgorithm(const gtsam::GaussianFactorGraph::shared_ptr& OmegaBar,
                                 const std::vector<gtsam::HessianFactor::shared_ptr>& Deltas,
  ...
  std::vector<size_t> ordering; ordering.reserve(N); // for lazy evaluation
  std::vector<double> upperBounds; upperBounds.reserve(N); // for lazy evaluation
  gtsam::Vector inserted = gtsam::Vector::Zero(N); // 0 = not inserted
  // run greedy
  double nrGainEval = 0;
  double gainOmegaBar = 0;
  double timeOrdering = 0, timeGreedy = 0; double startTime;
  for(size_t i = 0; i < need_n_corners; i++){ // for each feature we have to add

#ifdef FEATURE_SELECTOR_DEBUG_COUT
    startTime = UtilsOpenCV::GetTimeInSeconds();
#endif
    if(useLazyEval){
      std::tie(ordering,upperBounds,gainOmegaBar) = OrderByUpperBound(bestOmegaBar, Deltas, criterion);
    }else{
      ordering.clear(); upperBounds.clear();
      for(size_t j=0; j<N; j++){
    ...
    for(size_t indj=0; indj < N; indj++){ // loop over all available features and compute gain

      // check lazy stopping condition
      if(useLazyEval && (best_gain_j > upperBounds.at(indj)))
        break; // lazy evaluation is current best is better than sorted upper bound, then we can stop

      size_t j = ordering.at(indj); // make sure that we look according to descending upperbounds

      // std::cout << " best_gain_j " << best_gain_j << " best_j " << best_j << " j "<< j << " nr keys: " << Deltas.at(j).keys().size() << std::endl;
      if(inserted(j) == 0 && (Deltas.at(j)->keys().size() > 0 || !useLazyEval)){ // not inserted yet and Delta_j has some info
        gains(j) = EvaluateGain( bestOmegaBar, Deltas.at(j), criterion);
        nrGainEval += 1;
        // std::cout << " gains(j) " << gains(j) << " best_gain_j " << best_gain_j << " best_j " << best_j << " j "<< j << std::endl;
        // update max
        if(gains(j) > best_gain_j){
          best_j = j; best_gain_j = gains(j);
        }
    ...
    // if no feature won, pick the first that was not taken (features are ordered by quality)
    if(best_gain_j == numericalLowerBound){
      for(size_t j = 0; j < N; j++){
        if(inserted(j) == 0){
          best_j = j; break; // pick first nonzero
        }
      }
    }
    //std::cout << "gains.sortDescending(): " << gains.sortDescending() << std::endl;
    ...
    selectedMarginalGains.push_back(best_gain_j - gainOmegaBar); // marginal gains
...
#ifdef FEATURE_SELECTOR_DEBUG_COUT
  std::cout << "-- -- greedyAlgorithm: Ordering time: " << timeOrdering  << std::endl;
  std::cout << "-- -- greedyAlgorithm: Greedy time: " << timeGreedy << std::endl;
#endif
...
/* ------------------------------------------------------------------------ */
boost::tuple<int, double, gtsam::Vector>
FeatureSelector::SmallestEigsPowerIter(const gtsam::Matrix& M){
  ...
  bool reachedMaxIter = true; double normx;
  for(size_t iter = 0; iter < maxIter; ++iter){
    x = M*x;
    normx = x.norm();
    x = x / normx;
    if( (x-xold).norm() < tol){
      std::cout << "Largest eig: " << normx << "(iters: " << iter << ")" << std::endl;
      reachedMaxIter = false; break;
    }
    xold = x;
  }
  double lambdaMax = normx;
  if (reachedMaxIter)
    std::cout << "LargestEigsFast: reached maximum number of iterations" << std::endl;
  ...
  for(size_t iter = 0; iter < maxIter; ++iter){
    ...
    x = x / normx;
    if( (x-xold).norm() < tol){
      std::cout << "Smallest eig: " << normx << "(iters: " << iter << ")" << std::endl;
      reachedMaxIter = false; break;
    }
    xold = x;
  }
  if (reachedMaxIter)
    std::cout << "SmallestEigsPowerIter: reached maximum number of iterations" << std::endl;
...
/* ------------------------------------------------------------------------ */
boost::tuple<int, double, gtsam::Vector>
FeatureSelector::SmallestEigsSpectra(const gtsam::Matrix& M){
#ifdef useSpectra
  ...
  Spectra::SymEigsSolver< double, Spectra::SMALLEST_MAGN,
      Spectra::DenseSymMatProd<double> > eigs(&op, 1, round(M.cols()/2)); //
...
FeatureSelector::SmallestEigsSpectraShift(const gtsam::Matrix& M){
  ...
  Spectra::SymEigsShiftSolver< double, Spectra::LARGEST_MAGN,
      Spectra::DenseSymShiftSolve<double> > eigs(&op, 1, round(M.cols()/2), 0.0);
  eigs.init();
...
double FeatureSelector::EvaluateGain(
  ...
  int rank; gtsam::Vector eigVector;
  switch(criterion)
  {
    case VioFrontEndParams::FeatureSelectionCriterion::MIN_EIG :  // picks the best features that maximize the smallest eigenvalue of the covariance
      if(!useDenseMatrices){
        std::cout << "EvaluateGain: useDenseMatrices is deprecated" << std::endl;
        // NOTE (Luca): 2x slower than SmallestEigs
        boost::tie(rank,gain,eigVector) = SmallestEigsPowerIter(OmegaBar->hessian().first);
        // boost::tie(rank,gain,eigVector) = SmallestEigsSpectra(OmegaBar->hessian().first);
        //boost::tie(rank,gain,eigVector) = SmallestEigsSpectraShift(OmegaBar->hessian().first);
      }else{ // use eigen with dense matrices
        boost::tie(rank,gain,eigVector) = SmallestEigs(OmegaBar->hessian().first);
      }
      break;
    case VioFrontEndParams::FeatureSelectionCriterion::LOGDET :  // picks the best features that maximize the logdet of the covariance
      if(!useDenseMatrices){
        std::cout << "EvaluateGain: useDenseMatrices is deprecated" << std::endl;
        // NOTE (Luca): this seems slightly slower than Logdet(OmegaBar->hessian().first): 0.032 vs 0.030
        // also, sparse version sometimes gives: terminate called after throwing an instance of 'gtsam::InconsistentEliminationRequested'
        // what():  An inference algorithm was called with inconsistent arguments.  The factor graph, ordering, or variable index were inconsistent with each
        // other, or a full elimination routine was called with an ordering that does not include all of the variables.
        bayesTree = OmegaBar->eliminateMultifrontal();
        gain = 2*bayesTree->logDeterminant();
        // pow(bayesTree->determinant(),2); // Bayes tree determinant returns det(R), where R'R = our hessian
      }else{ // use eigen with dense matrices
        gain = Logdet(OmegaBar->hessian().first);
        // throw std::runtime_error("EvaluateGain: det computation leads to very large numbers");
        // gain = OmegaBar_U_Deltaj.hessian().first.determinant();
      }
      break;
    default:
...
FeatureSelector::getCameras(
  ...
  // create cameras
  Cameras left_cameras, right_cameras;
  left_cameras.reserve(nrKeyframesInHorizon); right_cameras.reserve(nrKeyframesInHorizon);
  for(size_t i=0; i < nrKeyframesInHorizon; i++)
  {
    const gtsam::Pose3 bodyPose = posesAtFutureKeyframes.at(i).pose;
    left_cameras.push_back(Camera(
                             bodyPose.compose(featureSelectionData.body_P_leftCam), featureSelectionData.left_undistRectCameraMatrix));
    right_cameras.push_back(Camera(
                              bodyPose.compose(featureSelectionData.body_P_rightCam),featureSelectionData.right_undistRectCameraMatrix));
  }
...
gtsam::JacobianFactor FeatureSelector::createPrior(
  ...
  const StampedPose poseStamped0 = featureSelectionData.posesAtFutureKeyframes.at(stateIndex);
  ...
  gtsam::noiseModel::Gaussian::shared_ptr noisePrior = gtsam::noiseModel::Gaussian::Covariance(covariancePrior);

  // Since GTSAM only supports diagonal noise model with Jacobians, we have to whiten
  // ||x - xBar||_Omega = ||sqrtOmega x - sqrtOmega xBar ||
  gtsam::Matrix A_prior = gtsam::Matrix9::Identity();
  noisePrior->WhitenInPlace(A_prior); // Now A_prior contains the sq info matrix
...
/* ------------------------------------------------------------------------ */
std::pair<gtsam::Matrix, gtsam::Matrix> FeatureSelector::createMatricesLinearImuFactor(
  ...
  // sanity check
  if ( Deltaij < 0 )
    throw std::runtime_error("createMatricesLinearImuFactor: Deltaij is negative!");

  int nrImuMeasurements = round(Deltaij/imuDeltaT_);
  double imuRate_ij = double(Deltaij / nrImuMeasurements); // small adjustment to accounts the Deltaij is not multiple of imuDeltaT_

  // sanity check
  if(std::abs(imuRate_ij-imuDeltaT_)>imuDeltaT_)
    throw std::runtime_error("createOmegaBar: imuRate_ij too different from imuDeltaT_");
  if(std::abs(nrImuMeasurements*imuRate_ij-Deltaij)>1e-4) // we did all this to make Deltaij a multiple of imuRate_ij
    throw std::runtime_error("createOmegaBar: Deltaij inconsistent with imuRate_ij");

  // 1.1) create matrices Mij and Nij and Covariance: covImu = [accVarianceDiscTime_ *  CCt   0 ;  0  biasAccVarianceDiscTime_ * I33]
  ...
  gtsam::Matrix3 Rimu = gtsam::Rot3::Expmap(rotVec_ij / nrImuMeasurements).matrix();
  ...
  gtsam::Matrix3 Rh = Ri;
  for(size_t h = 0; h < nrImuMeasurements; h++){ // note wrt draft: k=0 and j-1 = nrImuMeasurements
    Mij += Rh;
  ...
  covImu.block<3,3>(0,0) = gtsam::I_3x3 * (nrImuMeasurements * integrationVar_ + CCt_11 * imuRate_ij_4 * accVarianceDiscTime_);
  covImu.block<3,3>(0,3) = gtsam::I_3x3 * CCt_12 * imuRate_ij_3 * accVarianceDiscTime_;
  covImu.block<3,3>(3,0) = covImu.block<3,3>(0,3).transpose();
  covImu.block<3,3>(3,3) = gtsam::I_3x3 * nrImuMeasurements * imuRate_ij_2 * accVarianceDiscTime_;
  covImu.block<3,3>(6,6) = gtsam::I_3x3 * nrImuMeasurements * biasAccVarianceDiscTime_; //  this depends on the imu bias random walk
...
gtsam::GaussianFactorGraph FeatureSelector::createOmegaBarImuAndPrior(
  ...
  // create OmegaBar: requires rotations at imu timestamps in horizon (we get this by interpolation of frame )
  gtsam::GaussianFactorGraph OmegaBar;
  ////////////////////////////////////////////////////////////////////////////////////////
  // 0) create prior on first state: note: here the state is in Real{9}: position, velocity, biasAcc
  ...
  for(size_t i = 0; i < nrKeyframesInHorizon-1; i++)
  {
    ...
    std::tie(Ai,covImu) = createMatricesLinearImuFactor(poseStamped_i,poseStamped_j);

    // create noise model
    gtsam::noiseModel::Gaussian::shared_ptr noiseImu = gtsam::noiseModel::Gaussian::Covariance(covImu);
    ...
    OmegaBar.push_back(gtsam::JacobianFactor(i, Ai, i+1, Aj, gtsam::Vector::Zero(9)));
  }
  return OmegaBar;
}
...
gtsam::HessianFactor::shared_ptr FeatureSelector::createLinearVisionFactor(
  ...
  size_t nrKeyframesInHorizon = std::min(left_cameras.size(), size_t ( keypointLife) );

  // create quantities to define hessian factor
  gtsam::FastVector<gtsam::Key> keys; keys.reserve(nrKeyframesInHorizon);
  std::vector<gtsam::Matrix69> FBlocks; FBlocks.reserve(nrKeyframesInHorizon); // 3 + 3 = 6 measurements
  ...
  if(debug) {std::cout << "createLinearVisionFactor: before loop " << std::endl;}
  // reproject in each camera and build corresponding Jacobian

#ifdef FEATURE_SELECTOR_DEBUG_COUT
  double startTime = UtilsOpenCV::GetTimeInSeconds();
#endif
  bool featureTrackInterrupted = false; // unfortunately, we always populate the data structures, even with zeros
  for(size_t c = 0; c < nrKeyframesInHorizon; c++)
  {
    ...
    boost::optional<gtsam::Unit3> uij_left =
        FeatureSelector::GetVersorIfInFOV(left_cameras.at(c),pworld_l,landmarkDistanceThreshold_);
    if(uij_left && !featureTrackInterrupted){ // if point was in field of view
      gtsam::Matrix3 uijx_RkRcamTran = sqrtInfoVision_ * uij_left->skew() * (left_cameras.at(c)).pose().rotation().matrix().transpose();
    ...
    if(!hasRightPixel || !useStereo_){continue;} // otherwise those blocks remain zeros
    // try to project point to left camera: if point is in FOV, add to Jacobians
    boost::optional<gtsam::Unit3> uij_right =
        FeatureSelector::GetVersorIfInFOV(right_cameras.at(c),pworld_l,landmarkDistanceThreshold_);
    if(uij_right && !featureTrackInterrupted){ // if point was in field of view
      gtsam::Matrix3 uijx_RkRcamTran = sqrtInfoVision_ *  uij_right->skew() * (right_cameras.at(c)).pose().rotation().matrix().transpose();
      FBlocks.at(c).block<3,3>(3,0) = - uijx_RkRcamTran;
      E.block<3,3>(6*c + 3,0) = uijx_RkRcamTran;
    }
  }
  if(debug && featureTrackInterrupted) { std::cout << "Feature track was broken before "<< nrKeyframesInHorizon << " keyframes" << std::endl;}
  if(debug) {std::cout << "createLinearVisionFactor: after loop " << std::endl;}
#ifdef FEATURE_SELECTOR_DEBUG_COUT
  ...
  if(debug) {std::cout << "Pinv \n " << Pinv << std::endl;}
  int rank; double minSv; gtsam::Vector eigVector;
  boost::tie(rank,minSv,eigVector) = SmallestEigs(Pinv);
  // std::cout << " minSv " << minSv << std::endl;
  if(minSv < 1e-9)
    return boost::make_shared<gtsam::HessianFactor>(); // point cannot be triangulated, factor is useless
...
gtsam::GaussianFactorGraph::shared_ptr FeatureSelector::createOmegaBar(
  ...
  if(debug) {std::cout << "createOmegaBar: createOmegaBar" << std::endl;}
  gtsam::GaussianFactorGraph OmegaBar = createOmegaBarImuAndPrior(featureSelectionData);
  if(testing){ if(OmegaBar.size() != left_cameras.size()){ throw std::runtime_error("createOmegaBar: createOmegaBarImuAndPrior returned wrong size "); } }

#ifdef FEATURE_SELECTOR_DEBUG_COUT
  std::cout << "-- createOmegaBar: createOmegaBar time: " << UtilsOpenCV::GetTimeInSeconds() - startTime << std::endl;
  startTime = UtilsOpenCV::GetTimeInSeconds();
#endif
  ...
  for(size_t l=0; l < featureSelectionData.keypoints_3d.size(); l++)
  {
    ...
    gtsam::Point3 p_l_camL0 = gtsam::Point3(featureSelectionData.keypoints_3d.at(l));
    // convert to global frame (point are expressed wrt camera 0)
    gtsam::Point3 pworld_l = left_cameras.at(0).pose() * p_l_camL0; // overload for tranform_from (converts to global frame)
    // add to graph
    gtsam::HessianFactor::shared_ptr H_l = createLinearVisionFactor(pworld_l,left_cameras,right_cameras,
                                                                    debugFETime, debugSVDTime, debugSchurTime,
...
#ifdef FEATURE_SELECTOR_DEBUG_COUT
  std::cout << "-- createOmegaBar: createLinearVisionFactor time : " << UtilsOpenCV::GetTimeInSeconds() - startTime  <<
               "(nr factors: " << featureSelectionData.keypoints_3d.size() << ")"<< std::endl;
  std::cout << "-- -- createLinearVisionFactor: debugFETime_ " << debugFETime << std::endl;
  std::cout << "-- -- createLinearVisionFactor: debugSVDTime_ " << debugSVDTime << std::endl;
  std::cout << "-- -- createLinearVisionFactor: debugSchurTime_ " << debugSchurTime << std::endl;
  std::cout << "-- -- createLinearVisionFactor: sum " << debugFETime + debugSVDTime + debugSchurTime << std::endl;
#endif

  if(debug) {std::cout << "createOmegaBar: done createLinearVisionFactor" << std::endl;}
  gtsam::HessianFactor OmegaBar_H = gtsam::HessianFactor(OmegaBar); // convert the factor graph into a single factor
  OmegaBar.resize(0);
...
std::vector<gtsam::HessianFactor::shared_ptr> FeatureSelector::createDeltas(
  ...
  for(size_t l=0; l < availableVersors.size(); l++)
  {
    ...
    if(distance < tol){ // if zero, i.e., if not available, we try to guess it:
      hasRightPixel = false; // we guess the distance, but only predict left measurements
      distance = featureSelectionDefaultDepth_; // start by setting to default
      for(size_t o = 0; o < featureSelectionData.keypoints_3d.size(); o++){ // search for a nearby known feature with valid depth
        double normo = featureSelectionData.keypoints_3d.at(o).norm();
        // if norm is nonzero and dot product = cos * depth is large enough
        if(normo > tol && gtsam::dot(versor_l,featureSelectionData.keypoints_3d.at(o)) > featureSelectionCosineNeighborhood_ * normo){
          // product is cos * norm, therefore the inequality asks for the sin to be > featureSelectionCosineNeighborhood_
    ...
    double debugFETime = 0; double debugSVDTime = 0; double debugSchurTime = 0;
    Deltas.push_back(createLinearVisionFactor(pworld_l,left_cameras,right_cameras,
                                              debugFETime, debugSVDTime, debugSchurTime, 1e9, // infinite life
                                              hasRightPixel));
  }
  return Deltas;
}
...
boost::optional<gtsam::Unit3> FeatureSelector::GetVersorIfInFOV(
  ...
  if(px.x() < 0 || px.y() < 0 || px.x() > 2*cam.calibration().px() || px.y() > 2*cam.calibration().py()){
...
gtsam::SymmetricBlockMatrix FeatureSelector::SchurComplement(
  ...
  dims.back() = 1;
  gtsam::SymmetricBlockMatrix augmentedHessian(dims, gtsam::Matrix::Zero(M1, M1));
...
std::pair<SmartStereoMeasurements,double> FeatureSelector::splitTrackedAndNewFeatures_Select_Display(
  ...
  // tracked features (observed in exacly 2 kf - after 1 kf they are uninformative)
  ...
  if (newSmartStereoMeasurements.size() > need_nr_features) {
    ...
    if (criterion == VioFrontEndParams::FeatureSelectionCriterion::QUALITY) {
      ...
      if(newlyAvailableKeypointsScore.size() != newSmartStereoMeasurements.size())
        throw std::runtime_error("stereoVioExample: wrong score vector size");

      for(size_t ni = 0; ni < newSmartStereoMeasurements.size(); ++ni ){
        corners.push_back(KeypointCV(newSmartStereoMeasurements.at(ni).second.uL(),
                                     newSmartStereoMeasurements.at(ni).second.v())); // undistorted, rectified
      ...
      cam_param.distortion_coeff_ = cv::Mat::zeros(1, 5, CV_64F); //undistorted points

      std::cout << "selector: calling selector" << std::endl;
      double startTime = UtilsOpenCV::GetTimeInSeconds();
      std::tie(corners,selectedIndices,selectedGains) = featureSelectionLinearModel(
