...
static double numericalLowerBound = -numericalUpperBound; // min instead will return a tiny positive number
...
public:
  ...
  // default constructor
  FeatureSelectorData() : body_P_leftCam(gtsam::Pose3()), body_P_rightCam(gtsam::Pose3()),
      left_undistRectCameraMatrix(gtsam::Cal3_S2()),right_undistRectCameraMatrix(gtsam::Cal3_S2()) {}
  ...
  bool isEmpty() const{ return (posesAtFutureKeyframes.size() == 0); } // empty if there are no poses in horizon
};
