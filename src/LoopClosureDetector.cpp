...
LoopClosureDetectorOutputPayload LoopClosureDetector::checkLoopClosure(
  ...
  if (loop_result.detection()) {
    ...
    // TODO: this isn't working. But is it even necessary?
    // rewriteStereoFrameFeatures(&stereo_frame, keypoints);

    gtsam::Pose3 match_pose_curr = calcScaledRelativePose(&stereo_frame,
      &loop_result);
...
void LoopClosureDetector::rewriteStereoFrameFeatures(
    StereoFrame* stereo_frame, std::vector<cv::KeyPoint> keypoints) {
...
gtsam::Pose3 LoopClosureDetector::calcScaledRelativePose(
    StereoFrame* stereo_frame, DLoopDetector::DetectionResult* loop_result) {
  ...
  gtsam::Vector3 unit_T = gtsam::Vector3(
    loop_result->translation.at<double>(0),
  ...
  gtsam::Pose3 scaledRelativePose = gtsam::Pose3(rot, scaled_T);
