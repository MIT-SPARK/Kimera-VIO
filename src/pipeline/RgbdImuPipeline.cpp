...
RgbdImuPipeline::RgbdImuPipeline(const VioParams& params,
  ...
  vio_frontend_module_->registerOutputCallback([&backend_input_queue](
    ...
    if (converted_output && converted_output->is_keyframe_) {
      ...
      backend_input_queue.push(VIO::make_unique<BackendInput>(
          ...
          converted_output->relative_pose_body_rgbd_));
    } else {
  ...
  vio_backend_module_->registerImuBiasUpdateCallback(
   ...
   // TODO(saching): Disabling mesher for now since it needs the StereoFrontendOutput structure as input. 
   // Will enable it back later.
