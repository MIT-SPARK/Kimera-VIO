...
void VioBackend::addStateValues(const FrameId& frame_id,
  ...
  // NOTE: we use the latest state instead of W_Pose_B_lkf_ because that one is 
  // generated by chaining relative poses from the optimization, and is 
  // therefore much smoother than the state coming from the optimizer.
  // Initializing the smoother_ optimization with W_Pose_B_lkf_ would 
  // cause crashes because it's different from the latest state in smoother_.
  gtsam::Pose3 W_Pose_B_lkf_from_state =
      state_.at<gtsam::Pose3>(gtsam::Symbol(kPoseSymbolChar, last_kf_id_));
...
void VioBackend::updateStates(const FrameId& cur_id) {
  ...
  W_Pose_B_lkf_ = W_Pose_B_lkf_.compose(B_lkf_Pose_kf);
