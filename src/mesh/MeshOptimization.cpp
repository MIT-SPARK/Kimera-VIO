...
#include "kimera_vio/mesh/MeshOptimization.cpp"
...
namespace VIO {
 ...
 MeshOptimization::MeshOptimization(const MeshOptimizerType& solver_type, const bool& debug_mode)
  ...
  void MeshOptimization::draw2dMeshOnImg(
  ...
  void MeshOptimization::draw3dMesh(const std::string& id,
    ...
    draw3dMesh(id,
               cv::Mat(1u, mesh_3d.getNumberOfUniqueVertices(), CV_8UC3, color),
               mesh_3d,
               display_as_wireframe,
               opacity);
  }
  ...
  void MeshOptimization::collectTriangleDataPoints(const cv::Mat& noisy_point_cloud,
                                 const Mesh2D& mesh_2d,
    ...
    for (size_t tri_idx = 0; tri_idx < mesh_2d.getNumberOfPolygons();
      ...
      switch (mesh_optimizer_type_) {
        case MeshOptimizerType::kGtsamMesh: {
          ...
          // TODO(TONI): damn this is nonlinear :( the things below don't
          // apply, and won't work
          if (false) {
            ...
            const gtsam::Vector3 kSpringRestLength(0, 0, 0);
            constexpr double kSpringConstant = 1.0;
            const gtsam::Matrix33 A1 = gtsam::eye(3, 1) * kSpringConstant;
            const gtsam::Matrix33 A2 =
                -1.0 * gtsam::eye(3, 1) * kSpringConstant;
      ...
      case MeshOptimizerType::kGtsamMesh: {
        ...
        for (size_t k = 0u; k < mesh_2d.getNumberOfPolygons(); k++) {
          ...
          for (const Mesh2D::VertexType& vtx_2d : poly_2d) {
            ...
            cv::viz::Color cov_color = rainbowColorMap(variance_of_depth);
  ...
  void MeshOptimization::getBearingVectorFrom2DPixel(const gtsam::Cal3_S2& intrinsics,
  ...
  void MeshOptimization::drawPointCloud(const std::string& id, const cv::Mat& pointcloud) {
