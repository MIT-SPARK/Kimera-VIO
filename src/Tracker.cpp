...
Tracker::featureDetection(const Frame &cur_frame,
  ...
  for (size_t i = 0; i < cur_frame.keypoints_.size(); ++i) {
    if (cur_frame.landmarks_.at(i) != -1) {
      cv::circle(mask, cur_frame.keypoints_.at(i), tracker_params.min_distance_,
...
void Tracker::featureTracking(Frame *ref_frame, Frame *cur_frame) {
  ...
  if (px_cur.size() > 0) {
    ...
    cv::calcOpticalFlowPyrLK(
        ref_frame->img_, cur_frame->img_, px_ref, px_cur, status, error,
      ...
      for (size_t i = 0, n = 0; i < indices.size(); ++i) {
        ...
        if (!status[i] ||
          ...
          ref_frame->landmarks_[i_ref] =
              -1; // we are marking this bad in the ref_frame since features in
                  // the ref frame guide feature detection later on
          continue;
        }
...
Tracker::geometricOutlierRejectionMonoGivenRotation(Frame *ref_frame,
  ...
  std::vector<int> inliers = cv::ransac_2_point(
      ...
      trackerParams_.ransac_threshold_mono_, trackerParams_.ransac_probability_,
...
Tracker::geometricOutlierRejectionStereoGivenRotation(
  ...
  // double timeMahalanobis = 0, timeAllocate = 0, timePushBack = 0, timeMaxSet
  // = 0;
  float threshold =
...
void Tracker::findMatchingStereoKeypoints(
  ...
  findMatchingKeypoints(ref_stereoFrame.getLeftFrame(),
                        cur_stereoFrame.getLeftFrame(), &matches_ref_cur_mono);
  findMatchingStereoKeypoints(ref_stereoFrame, cur_stereoFrame,
...
cv::Mat Tracker::displayFrame(const Frame &ref_frame, const Frame &cur_frame,
  ...
  for (size_t i = 0; i < cur_frame.keypoints_.size(); ++i) {
    ...
    if (cur_frame.landmarks_.at(i) == -1) { // Untracked landmarks are red.
      ...
      if (it != ref_frame.landmarks_.end()) {
        // If feature was in previous frame, display tracked feature with green
        // circle/line
  ...
  if (verbosity == 1) { // otherwise just return the image
    ...
    std::string img_name = folderName + "/trackerDisplay" + extraString + "_" +
